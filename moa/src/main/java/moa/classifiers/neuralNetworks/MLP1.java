...
public class MLP1 extends AbstractClassifier implements MultiClassClassifier, Regressor {
...
//			this.window.setClassIndex(context.classIndex());
    ...
    public void trainOnInstanceImpl(Instance inst) {
		...
		for(int i=0; i < inst.numInputAttributes(); i++){
			featureValues[i] = (float) inst.value(i);
		}
...
//			return new ImmutableCapabilities(Capability.VIEW_STANDARD);
	...
	private void initializeNetwork(Instance inst) {
		...
		featureValues = new float [inst.numInputAttributes()];
...
//		Tracker learningRateTracker = Tracker.fixed(0.03f);
		...
		try {
			nnmodel = Model.newInstance("mlp");
			// Construct neural network and set it in the block
			Block block = new Mlp(inst.numInputAttributes(), inst.numClasses(), new int[] {1024});
//		Block block = new Mlp(inst.numInputAttributes(), inst.numClasses(), new int[] {2});
...
//				.addTrainingListeners(TrainingListener.Defaults.logging());
			...
			trainer.initialize(new Shape(1, inst.numInputAttributes()));
		}catch (Exception e) {
