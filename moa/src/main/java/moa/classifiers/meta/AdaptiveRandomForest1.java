 ...
 * @version $Revision: 1 $
 */
public class AdaptiveRandomForest1 extends AbstractClassifier implements MultiClassClassifier,
                                                                        CapabilitiesHandler {
    ...
    public void resetLearningImpl() {
        ...
        if(numberOfJobs != AdaptiveRandomForest1.SINGLE_THREAD && numberOfJobs != 1)
...
//        BasicClassificationPerformanceEvaluator classificationEvaluator = (BasicClassificationPerformanceEvaluator) getPreparedClassOption(this.evaluatorOption);
        ...
        switch(this.mFeaturesModeOption.getChosenIndex()) {
            case AdaptiveRandomForest1.FEATURES_SQRT:
                this.subspaceSize = (int) Math.round(Math.sqrt(n)) + 1;
                break;
            case AdaptiveRandomForest1.FEATURES_SQRT_INV:
                this.subspaceSize = n - (int) Math.round(Math.sqrt(n) + 1);
                break;
            case AdaptiveRandomForest1.FEATURES_PERCENT:
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
        if (this.getClass() == AdaptiveRandomForest1.class)
