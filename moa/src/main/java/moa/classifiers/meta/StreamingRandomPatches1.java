 ...
 * @version $Revision: 1 $
 */
public class StreamingRandomPatches1 extends AbstractClassifier implements MultiClassClassifier,
        CapabilitiesHandler {
    ...
    protected void initEnsemble(Instance instance) {
        ...
        if(this.trainingMethodOption.getChosenIndex() != StreamingRandomPatches1.TRAIN_RESAMPLING) {
            ...
            switch(this.subspaceModeOption.getChosenIndex()) {
                case StreamingRandomPatches1.FEATURES_SQRT:
                    k = (int) Math.round(Math.sqrt(n)) + 1;
                    break;
                case StreamingRandomPatches1.FEATURES_SQRT_INV:
                    k = n - (int) Math.round(Math.sqrt(n) + 1);
                    break;
                case StreamingRandomPatches1.FEATURES_PERCENT:
            ...
            if(this.trainingMethodOption.getChosenIndex() == StreamingRandomPatches1.TRAIN_RANDOM_SUBSPACES ||
                    this.trainingMethodOption.getChosenIndex() == StreamingRandomPatches1.TRAIN_RANDOM_PATCHES) {
                if(k != 0 && k < n) {
                    ...
                    if(n <= 20 || k < 2) {
                        ...
                        this.subspaces = StreamingRandomPatches1.allKCombinations(k, n);
                    ...
                    // subspaces without worrying about repetitions.
                        ...
                        this.subspaces = StreamingRandomPatches1.localRandomKCombinations(k, n,
                ...
                // k == 0 or k > n (subspace size greater than the total number of features), then default to resampling
                    ...
                    this.trainingMethodOption.setChosenIndex(StreamingRandomPatches1.TRAIN_RESAMPLING);
                }
            }
        }
        ...
        for(int i = 0 ; i < ensembleSize ; ++i) {
            switch(this.trainingMethodOption.getChosenIndex()) {
                case StreamingRandomPatches1.TRAIN_RESAMPLING:
                    this.ensemble[i] = new StreamingRandomPatchesClassifier(
                            ...
                            false);
                    break;
                case StreamingRandomPatches1.TRAIN_RANDOM_SUBSPACES:
                case StreamingRandomPatches1.TRAIN_RANDOM_PATCHES:
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
        if (this.getClass() == StreamingRandomPatches1.class)
